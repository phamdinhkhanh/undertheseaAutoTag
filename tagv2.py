...
# logging.info('existing file: {}'.format(os.getcwd()))
...
class TagProduct(object):
    def __init__(self, modelVersion = '0.0.2', apiType = 'oldProduct', **kwargs):
        self.modelVersion = modelVersion
        self.apiType = apiType
        if self.modelVersion == '0.0.2':
            if self.apiType == 'oldProduct':
                #__init__(self, productCode, modelVersion == '0.0.2')
                self._initialize002(kwargs['productCode'])
                self.tagProduct = TagProduct002(self.productCode, self.modelVersion)
            elif self.apiType == 'newProduct':
                # __init__(self, productName, cateName, branchName, description, modelVersion = '0.0.2')
                self._initialize002NewProduct(kwargs['productName'], kwargs['cateName'], kwargs['branchName'],
                                              kwargs['description'])
                self.tagProduct = TagProduct002NewProduct(self.productName, self.cateName, self.branchName,
                                                          self.description, modelVersion=self.modelVersion)
        else:
            self.tagProduct = None

    def _initialize002(self, productCode, modelVersion = '0.0.2'):
        self.productCode = productCode
        self.modelVersion = modelVersion

    def _initialize002NewProduct(self, productName, cateName, branchName, description, modelVersion = '0.0.2'):
...
class TagProduct002(TagProduct):
    def __init__(self, productCode, modelVersion = '0.0.2'):
        self.productCode = productCode
        self.modelVersion = modelVersion
        self.link = args.link+'&fq=code_string:'+self.productCode
    ...
    # Tạo bảng dữ liệu gồm productCode, productName, descriptions
    ...
    def _extract_good_sents(self, text):
        '''Note: các item trong sentiments và text đã mặc định đưa về lower'''
    ...
    # Cập nhật trường sentiment vào dataset
    def _update_sentiments(self, dataset):
        ...
        return dataset
    # def _update_sentiments1(self, dataset):
    #     sentiments = []
    #     start = datetime.now()
    #     for i, p in enumerate(dataset['description']):
    #         if (i % 1000 == 0) & (i != 0):
    #             end = datetime.now()
    #             print('epochs: {}, time executing: {}'.format(int(i / 1000), end - start))
    #             start = end
    #         try:
    #             sentiments.append(self._extract_good_sents(p))
    #         except:
    #             sentiments.append('')
    #     logging.info(type(p))
    #     logging.info('sentiments: {}'.format(sentiments))
    #     dataset['sentiments'] = sentiments
    #     return dataset
    ...
    def _combine_tags(self, dataset):
        ...
        dataset['tags'] = [set(item) for item in keyword_suggestions]
        return dataset

class TagProduct002NewProduct(TagProduct002):
    def __init__(self, productName, cateName, branchName, description,  modelVersion = '0.0.2'):
        self.productName = productName
        self.modelVersion = modelVersion
        self.cateName = cateName
        self.branchName = branchName
    ...
    def _create_dataset(self):
        dataset = pd.DataFrame({'productName': self.productName,
                                'cateName': self.cateName, 'branchName': self.branchName,
                                'description': self.description})

        dataset.drop_duplicates(subset=['productName'], inplace=True)
        dataset.set_index(np.arange(dataset.shape[0]))
        logging.info('Dataset shape: {}'.format(dataset.shape))
        dataset = self._update_sentiments(dataset)
        # Cập nhật tag 1
